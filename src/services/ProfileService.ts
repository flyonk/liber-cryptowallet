...
class ProfileService implements IProfileService, IKycService, IVerificator {
  ...
  async getProfile() {
    const url = `${this.url}`;
    const res: TProfile | TErrorResponse =
  ...
  async updateProfile(data: TUpdateProfile) {
    const url = `${this.url}`;
    const res: TProfile | TErrorResponse =
      await this._apiServiceInstance.fetch.patch(url, data);
    return res;
  }
  ...
  async updateProfileAvatar(data: File) {
    ...
    const res: TSuccessResponse | TErrorResponse =
      await this._apiServiceInstance.fetch.patch(url, data, config);
    return res;
  }
  ...
  async kycCreateClaim(data: any) {
    ...
    const res: TClaimFileList | TErrorResponse =
  ...
  async kycDeleteFileById(id: number, fileId: number) {
    ...
    const res: TSuccessResponse | TErrorResponse =
      await this._apiServiceInstance.fetch.delete(url);
    return res;
  }
  ...
  async configureApp(): Promise<TConfigureAppData | TErrorResponse> {
    ...
    const res: TConfigureAppData | TErrorResponse =
      await this._apiServiceInstance.fetch.get(url);
    return res;
  }
  ...
  async verificationBySMS(data: {
    ...
    const res: TVerification | TErrorResponse =
      await this._apiServiceInstance.fetch.post(url, data);
    return res;
  }
